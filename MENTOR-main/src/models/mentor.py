...
class MENTOR(GeneralRecommender):
    ...
    def forward(self, interaction,adj,train=False):
        ...
        #得到扩展图的id
        #extended_id_embeds = self.conv_ui(adj, self.id_rep[:self.n_users],self.id_rep[self.n_users:])
        ...
        fine_grained_image = torch.multiply(self.image_behavior(extended_id_embeds), (explicit_image_embeds - coarse_grained_embeds)) + coarse_grained_embeds
        fine_grained_text = torch.multiply(self.text_behavior(extended_id_embeds), (explicit_text_embeds - coarse_grained_embeds)) + coarse_grained_embeds
        #integration_embeds = (fine_grained_image + fine_grained_text + coarse_grained_embeds) / 3
        self.v_rep = torch.cat((self.v_rep,fine_grained_image),dim=1)
        self.v_rep = self.max_pool(self.v_rep.unsqueeze(1)).squeeze(1) 
        
        self.t_rep = torch.cat((self.t_rep,fine_grained_text),dim = 1)
        self.t_rep = self.max_pool(self.t_rep.unsqueeze(1)).squeeze(1) 
       ...
       # self.user_rep = self.max_pool2(self.user_rep.unsqueeze(1)).squeeze(1)
        self.item_rep = item_rep
       # self.item_rep = self.max_pool2(self.item_rep.unsqueeze(1)).squeeze(1)
    ...
    def calculate_loss(self, interaction):
        ...
        # c_loss = self.InfoNCE(extended_it_user[user], integration_users[user], self.um_temp)
        # noise_loss_1 = self.cal_noise_loss(user, integration_users, self.um_temp)
        # noise_loss_2 = self.cal_noise_loss(user, extended_it_user, self.um_temp)
        ...
        return loss_value + reg_loss + align_loss + mask_g_loss + mask_f_loss
